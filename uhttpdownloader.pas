...
begin
  ...
  try
    ...
    if (HTTPSender.ResultCode >= 100) and (HTTPSender.ResultCode<=299)
      then
        begin
          HTTPSender.Document.SaveToFile(UTF8ToSys(TargetFile));
          Result := True;
        end
      else
        begin
          sMessage := 'HTTP.ResultCode: '+inttostr(HTTPSender.ResultCode);
        end;
  finally
    HTTPSender.Free;
  end;
...
begin
  ...
  if Reason = THookSocketReason.HR_ReadCount
    then
      begin
...
begin
  ...
  if Pos('Content-Length:', Header) <> 0
    then
      begin
